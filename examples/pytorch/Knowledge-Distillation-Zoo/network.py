...
def define_tsnet(name, num_class, cuda=True):
    if name == 'resnet20':
        ...
        net = mcccnn8(num_class=num_class)
...
class resnet20(nn.Module):
    def __init__(self, num_class):
        ...
        self.conv1   = nn.Conv2d(3, 192, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1     = nn.BatchNorm2d(192)
        #self.relu    = QuadActivation() #nn.ReLU()
        #self.relu    = lambda x: 0.125*x**2 + 0.5*x + 0.25
        self.relu    = nn.ReLU()

        self.res1 = self.make_layer(resblock, 3, 192, 192)
      #  self.res2 = self.make_layer(resblock, 3, 16, 32)
      #  self.res3 = self.make_layer(resblock, 3, 32, 64)

        self.avgpool = nn.AvgPool2d(8)
        self.fc      = nn.Linear(256*12, num_class)
    ...
    def forward(self, x):
      ...
      #  rb3 = self.res3(rb2[1])
        ...
        feat = self.avgpool(rb1[1])
        feat = feat.view(feat.size(0), -1)
        #print(feat.shape)
        out  = self.fc(feat)

        return None, None, None, None, feat, out
