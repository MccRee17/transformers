...
def HPO_S0():
    ...
    epoch_list = [50, 80, 100, 200, 3, 10, 30]
    best = None
    best_metric = 0
    ...
    for lr in lr_list:
        for bs in bs_list:
            for epoch in epoch_list:
                output_dir = os.path.join(base_dir, "HPO_S0" ,str(lr), str(bs), str(epoch))
                result_path = os.path.join(output_dir, "eval_results.json")
                cmd = f"python run_glue_scratch.py --model_name_or_path {model_path} \
                      --task_name {task_name} \
                ...
                if metric > best_metric:
                    ...
                    best_metric = metric
                with open(log_path, "a") as f:
                    f.write(f"fine-tuned S0 with lr {str(lr)} bs {str(bs)} epoch {epoch}, acc: {metric} \n")

    best_lr, best_bs, best_epoch = best
    with open(log_path, "a") as f:
        f.write(f"best S0 with lr {best_lr} bs {best_bs} epoch {best_epoch}, acc: {best_metric} \n")

def HPO_S1():
    lr_list = [1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4]
    bs_list = [256, 128, 64, 32, 16]
    epoch_list = [3, 10, 30, 50, 80, 100, 200]
    #lr_list = [1e-4]
    #bs_list = [8]
    best = None
    best_metric = 0
